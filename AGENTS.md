...
This is a Textimage template for creating contributed modules or themes. The project provides a complete development environment with CI configuration, testing setup, and deployment automation for publishing to Drupal.org.
...
- `textimage.*` - Template extension files
- `TextimageService.php` - Main service class template
...
1. Run `php init.php` to customize template for Textimage
2. Build environment: `make build` or `ahoy build`
3. Develop Textimage code in `src/`
...
- SQLite database created in `/tmp/site_textimage.sqlite`
